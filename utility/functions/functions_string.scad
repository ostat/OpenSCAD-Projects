// String utility functions below each have a small comment describing their behavior and parameters.

// Join function: Joins elements of a list into a single string with an optional delimiter.
...
// Substring function: Extracts a substring from a string `s` starting at index `b` and ending at index `e`.
substr = function(s,b,e) let(e=is_undef(e) || e > len(s) ? len(s) : e) (b==e) ? "" : join([for(i=[b:1:e-1]) s[i] ]);

// Split function: Splits a string `s` by a given separator into a list of substrings.
...
// Fixed-point formatting function: Formats a number `x` to a fixed width `w` with `p` decimal places, padding with `sp`.
...
// Floating-point string conversion function: Converts a string `s` to a floating-point number, handling various formats.
...
// CSV parsing function: Parses a CSV-formatted string `s` and converts each value to a floating-point number.
